...
-   **Beat Machine (Suno Integration)**: Production center Step 2 uses SunoAPI.org for real music/beat generation when `SUNO_API_KEY` is configured. Async generation with polling (`POST /api/production/generate-beat` → `GET /api/production/beat-status/:taskId`). Falls back to AI vocal engine (OpenAI gpt-audio) when no Suno key is present. Suno client at `server/suno-client.ts` handles generate, status polling, and audio download.
