...
- **Release Types**: `native` (emerald, paper trade minting with $1K cap and capacity meter) and `global` (blue, trust certificate with store link, no cap limit). Set via `release_type` column in `tracks` table. Native assets get MNT-977-xxx Mint IDs; Global assets get TRS-977-xxx Trust IDs.
