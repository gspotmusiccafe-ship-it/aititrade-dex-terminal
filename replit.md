...
- **Market Intelligence Module**: Autonomous "CEO Hands-Off" system powered by `server/market-governor.ts`. The `MarketGovernor` generates daily trading sessions with randomized parameters: Trading Rate (35-55%), Volatility (0-45%), Market Sentiment (BULL/BEAR/NEUTRAL). Auto-Pool Scaling assigns each asset a dynamic pool size ($500, $1K, or $2K) based on trading rate and velocity. 70/30 Liquidity Split (30% House Reserve / 70% Payout Pot) applied across all pools. Randomized Flash-Pool triggers schedule 2-minute warnings on ~25% of assets throughout the day. Session is date-seeded (deterministic per day). API: `/api/market/session` (full session data + pool configs), `/api/market/pool/:trackId` (per-track pool analytics). Backend `POST /api/orders` enforces dynamic pool ceilings. Frontend shows collapsible Market Intelligence panel on the exchange page.
