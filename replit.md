...
- **Release Types**: `native` (emerald, paper trade minting with $1K cap and capacity meter) and `global` (emerald Neural DNA theme, trust certificate with store link, no cap limit). Set via `release_type` column in `tracks` table. Native assets get MNT-977-xxx Mint IDs; Global assets get TRST-977-xxx Trust IDs.
...
- **User Management**: Includes free accounts and three trading tiers: Entry Trader ($4.99/mo — restricted, no pre-release/high-volatility), Exchange Trader ($24.99/mo — full market access + early pre-release), and Mint Factory CEO ($99 join + $9.99/mo — full access + minting rights + 16% daily trading credit). Old Bronze/Silver/Gold names fully scrubbed.
