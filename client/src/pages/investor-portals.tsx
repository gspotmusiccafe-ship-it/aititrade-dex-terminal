...
export default function AititradeDEX() {
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh', padding: '40px', color: '#0f0', fontFamily: 'monospace' }}>
      <h1 style={{ color: '#fff', borderBottom: '2px solid #f0f', paddingBottom: '10px' }}>AITITRADE <span style={{color: '#f0f'}}>DEX TERMINAL</span></h1>
          ...
          return (
            <div key={p.id} style={{ width: '320px', background: '#111', border: `2px solid ${color}`, borderRadius: '15px', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ fontSize: '32px', fontWeight: 'bold', color: color, margin: '10px 0' }}>${mkt.current_price}</div>
                <div style={{ height: '8px', background: '#222', borderRadius: '4px' }}>
                  <div style={{ width: `${mkt.current_pct}%`, height: '100%', background: color, boxShadow: `0 0 10px ${color}` }} />
                </div>
                <button onClick={() => trade(p.id)} style={{ width: '100%', marginTop: '20px', padding: '15px', background: color, border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>
      ...
      {audio && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, width: '100%', background: '#000', borderTop: '4px solid #f0f', padding: '20px' }}>
